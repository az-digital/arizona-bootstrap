...
{{< ourname >}} uses [npm scripts](https://docs.npmjs.com/misc/scripts) for its build system. Our [package.json]({{< param "repo" >}}/blob/v{{ site.current_version }}/package.json) includes convenient methods for working with the framework, including compiling code, running tests, and more.
...
2. Navigate to the root `/arizona-bootstrap` directory and run `npm install` to install our local dependencies listed in [package.json]({{< param "repo" >}}/blob/v{{ site.current_version }}/package.json).

## Using npm Scripts

Our [package.json]({{< param "repo" >}}/blob/v{{ site.current_version }}/package.json) includes the following commands and tasks:
...
We maintain the list of browsers supported through Autoprefixer in a separate file within our GitHub repository. See [.browserslistrc]({{< param "repo" >}}/raw/v{{ site.current_version }}/.browserslistrc) for details.
