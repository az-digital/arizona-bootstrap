...
COPY --from=openjdk:11.0.14.1-jre-slim-bullseye "$JAVA_HOME" "$JAVA_HOME"
COPY --from=openjdk:11.0.14.1-jre-slim-bullseye /etc/ca-certificates/update.d/docker-openjdk /etc/ca-certificates/update.d/docker-openjdk
...
RUN apt-get update \
  ...
  && npm install -g npm-check-updates@12.5.8 \
  && npm install \
